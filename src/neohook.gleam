...
fn make_handler(
  state: AppState,
) -> fn(request.Request(mist.Connection)) -> Response
...
type SseState {
  SseState(id: Int, pipe_name: String, master: pipemaster.Subject)
}
...
type CurlState {
  CurlState(id: Int, pipe_name: String, master: pipemaster.Subject)
}
...
fn listen_on_pipe_for_curl(
  ...
  http_wrapper.chunked(
    ...
    init: fn(receiver) {
      ...
      process.send(
        master,
        pipemaster.AddPipe(name: pipe_name, id: id, subject: pipe_subj),
      )
    ...
    loop: fn(state, data, conn) {
      case http_wrapper.send_chunk(conn, data) {
        ...
        Error(_) -> {
          process.send(
            state.master,
            pipemaster.CleanPipe(state.pipe_name, remove_id: state.id),
          )
...
fn listen_on_pipe_for_sse(
  ...
  http_wrapper.server_sent_events(
    ...
    init: fn(subject) {
      ...
      process.send(
        master,
        pipemaster.AddPipe(name: pipe_name, id: id, subject: subject),
      )
...
pub type Peer {
  ...
  RemotePeer(address: Atom, pipe_entry_name: Atom, db_sync_name: Atom)
}
...
fn send_to_pipe(req: Request, pipe_name: String, state: AppState) -> Response {
  let user = user.from_request(req, state.db, state.ulid_from_string)
  ...
  case http_wrapper.read_body(req, 1024 * 100 * 50) {
    Ok(request.Request(body:, ..)) -> {
      ...
      let response_body =
        bytes_tree.from_string(id_string)
        |> bytes_tree.append(<<"\n":utf8>>)
...
fn serve_api(api_path: List(String), req: Request, state: AppState) -> Response {
  case req.method, api_path {
    http.Get, ["pipe"] -> {
      ...
      process.spawn(fn() {
        let #(sql, with, expecting) =
          sql.pipe_entries_by_pipe(pipe:, limit: 10, offset: 0)
        let with = with |> list.map(parrot_to_pturso)
      ...
      let entries =
        process.receive(entries_subj, within: 1000)
        |> result.lazy_unwrap(fn() { json.array([], of: fn(_) { json.null() }) })
      let settings =
        process.receive(settings_subj, within: 1000)
    ...
    http.Post, ["pipe", "settings"] -> {
      ...
      use <- lazy_guard(
        when: result.is_error(body),
        return: bad_request(because: "body too big"),
      )
      let assert Ok(request.Request(body:, ..)) = body

      let new_flags = json.parse_bits(body, pipe.flags_update_decoder())
      use new_flags <- expect(
        new_flags,
        or_return: bad_request(because: "missing fields"),
      )
        ...
        pturso.query(sql, on: state.db, with:, expecting: decode.success(Nil))
      {
        ...
        Error(e) -> {
          ...
          |> response.set_header("content-type", "application/json")
          |> response.set_body(
            mist.Bytes(bytes_tree.from_string("database error")),
          )
        }
      }
    }
    ...
    _, _ -> {
      ...
      |> response.set_header("content-type", "application/json")
      |> response.set_body(
        mist.Bytes(bytes_tree.from_string("{\"error\":\"path not found\"}")),
      )
    }
  }
}
...
pub fn http_handler(req: Request, state: AppState) -> Response {
  ...
  use <- lazy_guard(
    when: req.path != scrubbed_path,
    return: redirect(to: scrubbed_path),
  )
  ...
  case request.path_segments(req) {
    ...
    parts -> {
      ...
      case
        req.method,
        infer_requester_type(from_headers: req.headers),
        pipe_name
      {
        http.Get, CurlRequester, Some(pipe_name) ->
          listen_on_pipe_for_curl(req, pipe_name, state.master)
        http.Get, SseRequester, Some(pipe_name) ->
          listen_on_pipe_for_sse(req, pipe_name, state.master)
        http.Get, UnknownRequester, Some(_) ->
          serve_html("static/view.html", status: 200)
...
fn infer_requester_type(
  from_headers headers: List(#(String, String)),
) -> RequesterType {
  ...
  let handler = fn(req) {
    case request.path_segments(req) {
      [".well-known", "acme-challenge", challenge_file] -> {
        let path =
          "/var/www/public/.well-known/acme-challenge/" <> challenge_file
        logging.log(
          logging.Info,
          "Attempting to serve " <> challenge_file <> " at " <> path,
        )
...
pub fn tell_nodes_where_were_at(
  db: pturso.Connection,
  peers: List(Peer),
  me: Peer,
) {
  ...
  |> list.each(fn(peer) {
    let peer_string = peer_node(peer)
    dict.get(all, peer_string)
    |> result.map(fn(message) { peer_send_db_sync(peer, message) })
  })
}

fn expect(
  value: Result(a, b),
  or_return default: fn() -> r,
  when_ok continue: fn(a) -> r,
) {
  case value {
    Ok(x) -> continue(x)
    Error(_) -> default()
...
fn process_sync_hint(
  ...
  case values {
    [_, ..] -> {
      let sql =
        string_tree.from_string("INSERT INTO ")
        |> string_tree.append(table)
        |> string_tree.append(" (")
        |> string_tree.append_tree(
          fields
      ...
      logging.log(
        logging.Info,
        "Sending " <> int.to_string(list.length(values)) <> " " <> table,
      )
    ...
    DbSyncExec(sql, with) -> {
      ...
      case pturso.query(sql, on: db, with:, expecting: decode.success(Nil)) {
        ...
        Error(err) -> {
          logging.log(
            logging.Error,
            "Bad SQL over sync? " <> string.inspect(err) <> " -- " <> sql,
          )
        }
      }
    }
...
pub fn main() {
  ...
  let db =
    pturso.connect(turso, "db", log_with: fn(entry) {
      logging.log(
        logging.Info,
        "SQL: "
          <> string.replace(entry.sql, each: "\n", with: " ")
          <> " ["
          <> int.to_string(entry.duration_ms)
          <> "ms]",
      )
    })
  ...
  process.spawn(fn() {
    case migrations.migrate(db, migrations.all_migrations()) {
      ...
      Ok(ran) ->
        logging.log(logging.Info, "Ran migrations: " <> string.join(ran, ", "))
      Error(#(ran, failed, error)) -> {
        case ran {
          [_, ..] as did_run ->
            logging.log(
              logging.Info,
              "Ran migrations: " <> string.join(did_run, ", "),
            )
          _ -> Nil
        }
        logging.log(
          logging.Error,
          "Failed to run " <> failed <> ": " <> string.inspect(error),
        )
      }
    }
  })
  ...
  let assert Ok(app_url) =
    case env.get("HOOK_URL") {
      Some(url) -> url
  ...
  process.spawn(fn() { db_send_loop(db, state.peers, me) })
  ...
  let _ = case app_url.scheme, app_url.host {
    Some("https"), Some(host) -> {
      ...
      let https_start =
        tls.parse_config(at: tls_config_path)
      ...
      case https_start {
        ...
        Error(config_failed) ->
          logging.log(
            logging.Error,
            "Bad tls config: " <> string.inspect(config_failed),
          )
      }
