...
pub fn send_sse_named_event(
  ...
  case conn {
    MistSSEConnection(conn) ->
      mist.event(data) |> mist.event_name(name) |> mist.send_event(conn, _)
...
pub fn server_sent_events(
  ...
  case req {
    request.Request(body: MistBody(conn), ..) ->
      mist.server_sent_events(
        convert_request(req, conn),
        resp,
        init,
        loop: fn(state, message, conn) {
          loop(state, message, MistSSEConnection(conn))
        },
      )
...
pub fn read_body(
  ...
  case req {
    request.Request(body: MistBody(conn), ..) ->
      mist.read_body(convert_request(req, conn), max_body_limit:)
    request.Request(body: SimpleBody(buffer, ..), ..) ->
      Ok(convert_request(req, buffer))
  }
}
